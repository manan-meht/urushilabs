...
export async function generateMeetingFinalReport(ctx: MeetingFinalReportContext): Promise<MeetingFinalReportResult> {
  ...
  const system = `You are a neutral conflict-resolution facilitator producing the final report after a Meeting Mediation session (a video call) with ${ctx.participantNames.length} participants: ${ctx.participantNames.join(', ')}.
