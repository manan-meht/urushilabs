/**
 * Server-only: generates the final report at the end of a Live Mediation (room-mode)
 * session. Mirrors src/lib/ai/together/finalReport.ts — same rules, same raw-fetch
 * pattern for Cloudflare Workers compatibility. Never import from client components.
 */
...
export interface RoomFinalReportContext {
  topic: string
...
export interface RoomFinalReportResult {
  report: RoomFinalReport
  inputTokens: number
  outputTokens: number
}

export async function generateRoomFinalReport(ctx: RoomFinalReportContext): Promise<RoomFinalReportResult> {
  const { OPENAI_API_KEY, OPENAI_MODEL, DEMO_MODE } = getEnv()

  if (DEMO_MODE) {
    return {
      report: {
        whatHappened: `${ctx.participantNames.join(' and ')} discussed ${ctx.topic} together, with Urushi listening in and stepping in occasionally to help clarify points and capture agreements.`,
        agreed: [{
...
${transcriptText}`
  ...
  if (!res.ok) {
    ...
    throw new Error(`OpenAI room final report failed (${res.status}): ${text}`)
  }
  ...
  try {
    ...
    throw new Error('OpenAI returned invalid JSON for room final report.')
  }

  const validated = FinalReportSchema.safeParse(parsed)
  if (!validated.success) {
    throw new Error(`Room final report schema validation failed: ${validated.error.message}`)
  }

  return {
    report: validated.data as RoomFinalReport & { safetyCategory: SafetyCategory },
